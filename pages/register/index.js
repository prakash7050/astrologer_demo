        ...
        if(validateData()) {
            ...
            try{
                ...
                if(loginValue.isOtpSent){
                    sentData = {mobile:loginValue.mobile,otp:loginValue.otp}
                }else{
                    sentData = {...loginValue}
                }
                console.log(sentData);
                const res = await context.registerUser(sentData)
                console.log(res,res.data,res.data.status,"<<<<<<<<<<<<<<<register")
                if(res.data.status === "success") {
                    notifyUser("Otp verified")
                    notifyUser("Register account successful")
                    ...
                    alert('Please Check Enter Details')
                }
            }catch(err){
                alert("Somethings went worng")
            }
        }
    ...
    const validateData = () => {
        ...
        if(!loginValue.isOtpSent){
            ...
            if(!loginValue.name){
                ...
                return false
            }else if(!phoneRegExp.test(loginValue.mobile)) {
                setError({...errors,mobile: true})
                return false
            }
            else if(!loginValue.user_type){
                setError({...errors,user_type: true})
                return false
            } 
    ...
    return (
        <View style={styles.container}>
            ...
            {!loginValue.isOtpSent &&
                <>
                    <View style={styles.inputContainer}>
                        ...
                        <TextInput
                            ...
                            onChangeText={(value) => setLoginValue({...loginValue,name: value})}
                    ...
                    <View style={styles.inputContainer}>
                        ...
                        <Select minWidth="200" value={loginValue.user_type} onValueChange={itemValue => setLoginValue({...loginValue,user_type: itemValue})} backgroundColor='#f5f6f8' borderColor='#f5f6f8'>
                            <Select.Item shadow={1} label="CUSTOMER" value={'customer'} />
                            <Select.Item shadow={2} label="ASTROLOGER" value={'astrologer'} />
                        </Select>
                        {error?.user_type &&
                            <Text style={{color: 'red',textAlign: 'left',width: '80%',fontSize: 12}}>Select correct user type.</Text>
                        }
                    </View>
                ...
                <Button text={loginValue.isOtpSent ? "Verify" : 'Register'} color={'blue'} borderRadius={10} height={50} width={'40%'} marginTop={20} onPress={()=>submitForm()} isLoading={context.btnLoading}></Button>
                <TouchableOpacity style={{flexDirection:'row',marginTop:5}} onPress={()=>props.navigation.replace('Login')}>
                    <View><Text style={{color:'black'}}>Have Already Account</Text></View>
                    <View><Text style={{borderBottomWidth:1,borderBottomColor:'green',color:'green'}}>Login</Text></View>
                </TouchableOpacity>
        </View>
    )
}
